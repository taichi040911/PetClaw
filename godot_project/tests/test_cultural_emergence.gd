...
const _CulturalEmergenceSystem = preload("res://scripts/culture/cultural_emergence_system.gd")
...
func test_create_artifact() -> bool:
	...
	if art.get("type") != CulturalEmergenceSystem.ArtifactType.FESTIVAL:
		push_warning("create_artifact: wrong type"); sys.free(); return false
	if art.get("name") != "HarvestFest":
		push_warning("create_artifact: wrong name"); sys.free(); return false
	if sys.total_artifacts_created != 1:
		push_warning("create_artifact: total_artifacts_created != 1")
		sys.free(); return false
	sys.free(); return true
