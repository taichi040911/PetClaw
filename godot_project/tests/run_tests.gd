## run_tests.gd — PetClaw 統合テストランナー
## 全テストスイートを順番に実行し、結果を集計する
## 実行: godot --headless --script tests/run_tests.gd
##
## 個別テスト実行:
##   godot --headless --script tests/test_save_load.gd
##   godot --headless --script tests/test_sub_molt_themes.gd
##   godot --headless --script tests/test_a2a_fallback.gd
...
func _init() -> void:
	print("╔══════════════════════════════════════╗")
	print("║     PetClaw Test Runner v1.0         ║")
