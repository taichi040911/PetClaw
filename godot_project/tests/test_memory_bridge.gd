...
func test_grief_stages() -> bool:
	...
	if not bridge.grief_states.has(1):
		...
		bridge.queue_free(); return false
...
func test_to_dict_from_dict() -> bool:
	...
	if not r.grief_states.has(2):
		push_warning("round-trip: grief_states missing"); bridge.queue_free(); r.queue_free(); return false
	if not r.trauma_markers.has(2):
		push_warning("round-trip: trauma_markers missing"); bridge.queue_free(); r.queue_free(); return false
	if not r.nostalgia_log.has(2) or r.nostalgia_log[2].is_empty():
		push_warning("round-trip: nostalgia_log missing"); bridge.queue_free(); r.queue_free(); return false
	if not r.personality_modifiers.has(2):
		push_warning("round-trip: personality_modifiers missing")
		bridge.queue_free(); r.queue_free(); return false
	bridge.queue_free(); r.queue_free(); return true
...
func test_nostalgia_trigger() -> bool:
	...
	if not bridge.nostalgia_log.has(3):
		push_warning("nostalgia: log key missing"); bridge.queue_free(); return false
	...
	if not entry.has("memory_summary") or not entry.has("timestamp"):
		push_warning("nostalgia: entry missing required keys"); bridge.queue_free(); return false
	# Verify 20-entry rolling cap
	for i: int in range(22):
		bridge._log_nostalgia(3, "memory %d" % i)
	if bridge.nostalgia_log[3].size() > 20:
		push_warning("nostalgia: exceeded 20-entry cap"); bridge.queue_free(); return false
	bridge.queue_free(); return true
...
func test_dream_synthesis() -> bool:
	...
	if bridge.dream_log[4][0].get("content") != dream_text:
		push_warning("dream_synthesis: logged content mismatch"); bridge.queue_free(); return false
	...
	if bridge.dream_log[4].size() > 10:
		push_warning("dream_synthesis: exceeded 10-entry cap"); bridge.queue_free(); return false
	bridge.queue_free(); return true
