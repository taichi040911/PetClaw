...
func test_form_team() -> bool:
	...
	if not orch.active_teams.has("team_0"):
		push_warning("form_team: team_0 missing"); orch.queue_free(); return false
	var team: Dictionary = orch.active_teams["team_0"]
	if team.get("members", []).size() != 3:
		push_warning("form_team: member count mismatch"); orch.queue_free(); return false
	if team.get("leader_id") != 1:
		push_warning("form_team: unexpected leader_id"); orch.queue_free(); return false
	if team.get("task") != "EXPLORATION":
		push_warning("form_team: unexpected task"); orch.queue_free(); return false
	orch.queue_free(); return true
...
func test_to_dict_from_dict() -> bool:
	...
	if r.active_teams.size() != orch.active_teams.size():
		...
		orch.queue_free(); r.queue_free(); return false
	...
	if not r.faction_preferences.has("alpha_pack"):
		...
		orch.queue_free(); r.queue_free(); return false
	orch.queue_free(); r.queue_free(); return true
